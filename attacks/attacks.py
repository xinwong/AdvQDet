...
from attacks.adaptive.ZOO import ZOO
...
def attack_loader(model, loader, model_config, attack_config):
    ...
    for i, (x, y, p) in enumerate(pbar):
        ...
        try:
            ...
            # if model.model.model(x).argmax(dim=1) != y: # 低精度
                x_adv = x
        ...
        # logits = model.model.model(x_adv) # 低精度
        preds = torch.argmax(logits, dim=1).detach().cpu().numpy().tolist()
        true = y.detach().cpu().numpy().tolist()

        y_true.extend(true)
        y_pred.extend(preds)
        # print("Run Accuracy: {} ".format(accuracy_score(y_true, y_pred)))
        ...
        total_queries.append(attacker.get_total_queries())
        if model_config["state"]["type"] == "no_op":
            average_queries = sum(total_queries) / len(total_queries) if total_queries else 0
            print("Average {} Total Queries:{}".format(len(total_queries), average_queries))
